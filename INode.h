...
public:
  ...
  /*文件默认权限*/
  static const unsigned int FILE_DEF_PER = IREAD | IWRITE | (IREAD >> 3) | (IWRITE >> 3) | (IREAD >> 6);
  /*目录默认权限*/
  static const unsigned int DIR_DEF_PER = IREAD | IWRITE | IEXEC | (IREAD >> 3) | (IEXEC >> 3) | (IREAD >> 6) | (IEXEC >> 6);
...
public:
  ...
  int i_addr[10]; /* 用于文件逻辑块号和物理号好转换的基本索引表 */

  int i_atime; /* 最后访问时间,int类型暂时还不会爆范围 */
  int i_mtime; /* 最后修改时间 */

  int i_no; /* 外存inode区中的编号 */
};
